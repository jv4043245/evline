...
import { assessMarketCandidate, partTraits, compactPartNumber, canSearchMarketItem } from '../../assets/js/market-comparison.js';
...
export async function researchMarketItems(items, sources) {
  ...
  const searches = await Promise.all(items.filter(canSearchMarketItem).flatMap(item => sources.map(async source => {
