...
import { fail, normalizeDocument, validateReady } from '../../../../../admin/documents/model.js';
...
export async function onRequestPost({ env, request, params }) {
  ...
  if (!doc || doc.status !== 'ready' || validateReady(normalizeDocument(JSON.parse(doc.data_json))).length) throw fail('Спочатку підготуйте й перевірте документ.');
  const actor = auditActor(request, env), now = new Date().toISOString();
  const prior = await env.DB.prepare('SELECT * FROM document_deliveries WHERE id=?').bind(doc.id).first();
  ...
  const acquired = prior
    ? await env.DB.prepare("UPDATE document_deliveries SET status='sending',created_at=?,actor=?,recipient=?,error=NULL WHERE id=? AND status='failed'").bind(now, actor, recipient, doc.id).run()
    : await env.DB.prepare("INSERT OR IGNORE INTO document_deliveries(id,document_id,created_at,actor,recipient,status) VALUES(?,?,?,?,?,'sending')").bind(doc.id, doc.id, now, actor, recipient).run();
  if (!acquired.meta.changes) throw fail('Відправлення цієї версії вже виконується.', 409);
  const payload = new FormData();
  payload.set('chat_id', recipient); payload.set('document', file, `EVLine-${order.order_number || 'order'}-v${doc.revision}.pdf`);
  payload.set('caption', `EVLine · ${order.order_number || 'Замовлення'}\nДоговір і специфікація для перевірки та підписання. Версія ${doc.revision}.`);
  let response, result;
  try {
    ...
    await env.DB.prepare("UPDATE document_deliveries SET status='unknown',error=? WHERE id=?").bind('Результат відправлення не підтверджено. Перевірте чат перед повторенням.', doc.id).run();
  ...
  if (result?.ok !== true) {
    ...
    await env.DB.prepare('UPDATE document_deliveries SET status=?,error=? WHERE id=?').bind(definitive ? 'failed' : 'unknown', definitive ? 'Telegram не прийняв документ.' : 'Telegram не підтвердив результат.', doc.id).run();
  ...
  await env.DB.prepare("UPDATE document_deliveries SET status='sent',message_id=?,error=NULL WHERE id=?").bind(String(result.result?.message_id || ''), doc.id).run();
  await recordAuditEvent(env, { actor, action: 'document.send', entity_type: 'order_document', entity_id: doc.id, entity_label: order.order_number, order_id: order.id, details: { revision: doc.revision, channel: 'telegram' } });
  return json({ ok: true });
}
