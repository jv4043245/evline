...
try {
  for (const width of [1440, 1024, 768, 390]) {
    ...
    offers.push({ ...offers[0], product_url: 'https://example.test/wrong-side', title: 'Фара ліва', part_type: 'oem', match_type: 'irrelevant', match_reason: 'Інша сторона деталі', price_uah: 100 });
    ...
    assert.doesNotMatch(await form.locator('.market-items').innerText(), /Несумісн|Фара ліва/);
    ...
    await form.locator('.market-feedback-notice [data-market-feedback-undo]').waitFor();
    assert.equal(await form.locator('.market-offer--exact').count(), 2);
    assert.equal(await form.locator('.market-offer--irrelevant').count(), 0);
    ...
    assert.equal(await form.locator('.market-offer--exact').count(), 3);
    ...
    await estimate.locator('[data-air-guide-result]').waitFor();
    assert.match(await estimate.locator('[data-air-guide-result]').innerText(), /Робочий орієнтир/);
    assert.match(await estimate.locator('[data-air-guide-result]').innerText(), /Діапазон/);
    assert.equal(await estimate.locator('[data-air-rate], [data-air-weight]').count(), 0);
    ...
    await lookup.locator('.market-offer--exact .market-offer__feedback summary').first().click();
    await lookup.locator('[data-market-feedback-save]').first().click();
    await lookup.locator('.market-feedback-notice [data-market-feedback-undo]').waitFor();
    assert.equal(await lookup.locator('.market-offer--exact').count(), 2);
    await lookup.locator('[data-market-feedback-undo]').click();
    ...
    market.can_search = false;
    await page.goto(`${origin}/admin/`);
    await page.locator('[data-open-order="smoke-order"]').first().click();
    await form.locator('[data-order-tab="market"]').click();
    await form.locator('.market-item').first().waitFor();
    assert.equal(await form.locator('.market-offer').count(), 0);
    assert.equal(await form.locator('[data-market-car]').isVisible(), true);
    assert.equal(await form.locator('[data-market-car]').inputValue(), '');
    assert.match(await form.locator('.market-items').innerText(), /Уточніть модель авто або артикул/);
    await page.screenshot({ path: `${output}/market-identity-${width}.png` });
    const identityOverflow = await form.locator('.market-search').evaluate(el => el.scrollWidth > el.clientWidth + 1);
    assert.equal(identityOverflow, false);
    order.car = 'ZEEKR';
    order.item_name = 'Бампер передній';
    const brandItem = { key: 'bumper', label: 'Бампер передній', car: 'ZEEKR', part_numbers: [] };
    market.summary.items = [brandItem];
    market.summary.vehicle_lookup = { status: 'not_configured' };
    market.offers = [{ ...offers[0], item_key: 'bumper', title: 'Бампер передній ZEEKR 001', part_number: '', match_type: 'probable', match_reason: 'Модель авто не підтверджено' }];
    market.can_search = true;
    await page.goto(`${origin}/admin/`);
    await page.locator('[data-open-order="smoke-order"]').first().click();
    await form.locator('[data-order-tab="market"]').click();
    await form.locator('[data-market-vehicle]').waitFor();
    assert.match(await form.locator('[data-market-vehicle]').innerText(), /ще не підключено/);
    assert.equal(await form.locator('.market-offer--probable').count(), 1);
    assert.equal(await form.locator('.market-price-group, .market-offer--exact').count(), 0);
    await page.screenshot({ path: `${output}/market-vin-inactive-${width}.png` });
    market.summary.vehicle_lookup = { status: 'resolved', source: '17VIN', car: 'ZEEKR 001 2024' };
    market.summary.items = [{ ...brandItem, car: 'ZEEKR 001 2024' }];
    await page.goto(`${origin}/admin/`);
    await page.locator('[data-open-order="smoke-order"]').first().click();
    await form.locator('[data-order-tab="market"]').click();
    await form.locator('[data-market-vehicle]').waitFor();
    assert.match(await form.locator('[data-market-vehicle]').innerText(), /Авто за VIN: ZEEKR 001 2024/);
    assert.equal(await form.locator('.market-search-details').evaluate(el => el.open), false);
    assert.equal(await form.locator('[data-market-research-root]').evaluate(el => el.scrollWidth > el.clientWidth + 1), false);
    await page.screenshot({ path: `${output}/market-vin-resolved-${width}.png` });
    await page.goto(`${origin}/admin/`);
    await page.locator('[data-market-lookup-open]').click();
    await lookup.locator('[name="vin"]').fill('LTEST0000R0000001');
    await lookup.locator('[name="query"]').fill('Бампер передній');
    await lookup.locator('[data-market-lookup-form] [type="submit"]').click();
    await lookup.locator('[data-market-vehicle]').waitFor();
    assert.match(await lookup.locator('[data-market-vehicle]').innerText(), /ZEEKR 001 2024/);
    ...
    await page.locator('[data-air-guide-result]').waitFor();
    assert.equal(await page.locator('[data-air-advanced]').evaluate(el => el.open), false);
    await page.screenshot({ path: `${output}/air-guide-${width}.png`, fullPage: true });
    await page.locator('[data-air-advanced] > summary').click();
